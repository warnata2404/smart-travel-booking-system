...
const form = useForm({
    origin_city_id: props.travelRoute.origin_city_id,
    destination_id: props.travelRoute.destination_id,
    distance: Number(props.travelRoute.distance),
    estimated_duration: props.travelRoute.estimated_duration,
    base_price: Number(props.travelRoute.base_price),
    status: props.travelRoute.status,
});
...
function submit() {
    form.put(route("travel-routes.update", props.travelRoute.id));
}
</script>

<template>
    <Head title="Edit Travel Route" />

    <div class="space-y-6">
        <AppPageHeader
            title="Edit Travel Route"
            description="Update travel route information."
        >
            <template #actions>
                <AppPageToolbar>
